...
//Подменю "Настройки", выбор размерности окна
...
//== Отрисовка ==
       ...
       mvprintw(2,window_max+1,"Score: [%d] ",score | COLOR_PAIR(3));
       ...
       mvaddch(hy, hx, '@');//Голова
       for (int i = 1; i<ntail; i++)
       { 
          if (i<(ntail-1)) { mvaddch(tailY[i],tailX[i], ' '); }//Тельце
            else {  mvaddch(tailY[i],tailX[i], '+'); }//Хвостик
       }
//===========       
         refresh();
         draw_window(); //
...
int main()
{
    ...
    init_pair(2,  COLOR_YELLOW,     COLOR_YELLOW); //Тело змеи
